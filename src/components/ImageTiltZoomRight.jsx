...
/**
 * ImageTiltZoomRight
 * - Entrance: slide in from RIGHT (x: 40 -> 0) + fade
 * - Hover tilt on the PARENT (prevents background halo)
 * - Click/tap to open fullscreen; tap anywhere / ESC to close
 *
 * Props:
 *  - src (string, required)
 *  - alt (string)
 *  - title? (string)          // optional label chip
 *  - className? (string)
 *  - rounded? (string)        // Tailwind radius class, default 'rounded-2xl'
 *  - border? (boolean)        // default true
 *  - shadow? (boolean)        // default true
 *  - maxTilt? (number)        // degrees, default 6
 *  - delay? (number)          // seconds, for staggering in lists (default 0)
 */
export default function ImageTiltZoomRight({
  src,
  alt = "",
  title,
  className = "",
  ...
  return (
    <>
      {/* Entrance animation wrapper: slide RIGHT -> LEFT */}
      <motion.div
